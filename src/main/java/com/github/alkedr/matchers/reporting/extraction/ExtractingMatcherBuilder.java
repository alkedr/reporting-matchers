...
//        return new ExtractingMatcherBuilder<>(name, getExtractor(), getMatcher());
    ...
    public ExtractingMatcherBuilder<T> is(Matcher<?> matcher) {
        return new ExtractingMatcherBuilder<>(getExtractor(), new Checks(ReportingMatcher.PresenceStatus.PRESENT, ReportingMatchersAdapter.toReportingMatcher(matcher)));
    }
